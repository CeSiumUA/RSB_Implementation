...
{
    public static class RightGrayScheme
    {
        public static byte[] Encrypt(byte[] array, byte[] key, int grayNumber = 4)
        {
            ...
            {
                var keyBit = keyBits[keyBits.Length - i - 1];
                ...
                for (int j = matchingBits.Length - 1; j >= 0; j--)
                {
        ...
        public static byte[] Decrypt(byte[] array, byte[] key, int grayNumber = 4)
        {
            ...
            for (int i = 0; i < keyBits.Length; i++)
            {
                ...
                for (int j = matchingBits.Length - 1; j >= 0; j--)
                {
                    ...
                    matchingBits[j] = matchingBits[j] ^ previousBit;
                    previousBit = temp;
                }
